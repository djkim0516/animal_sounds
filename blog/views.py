...
from Aniaml_demo.ISPL_DEMO import Model, Solver
model_A = Model('A')
sess = tf.Session()
...
def animalclassificationresult(request):
    ...
    myfile = request.FILES['soundfile']

    filesys = FileSystemStorage()  # 이미지 파일을 저장할때 쓰는 함수
    filename = filesys.save(myfile.name, myfile)
    ...
    if inp_wav.split(".")[1] == "mp3":
        ...
        sound = AudioSegment.from_mp3('./media/' + inp_wav)
        sound.export('./media/' + inp_wav.split(".")[0] + ".wav", format="wav")
        os.remove('./media/' + inp_wav)
        inp_wav = inp_wav.split(".")[0] + ".wav"
    elif inp_wav.split(".")[1] == "m4a":
        sound = AudioSegment.from_file('./media/' + inp_wav, "m4a")
        sound.export('./media/' + inp_wav.split(".")[0] + ".wav", format="wav")
        os.remove('./media/' + inp_wav)
        inp_wav = inp_wav.split(".")[0] + ".wav"
    # inp_wav = os.path.join(os.listdir('./media/temporary')[0])
    ...
    prev_state = None
    with open("./Aniaml_demo/list_115.txt", "r", encoding='UTF8') as f:
        labels = f.readlines()
    ...
    class_convert = {60 : 0, 62 : 1, 63 : 2, 66 : 3, 67 : 4, 68 : 5, 70 : 6, 71 : 7, 69 : 8}
    inv_class_convert = [60, 62, 63, 66, 67, 68, 70, 71, 69]
    ...
    animal_data = []
    animal_color = []
    curr_state = os.path.getatime("./media/" + inp_wav)
    if curr_state != prev_state:
        print(time_string(), end=' ')
        print("New recorded file arrived!")
        prev_state = curr_state

        myrecording, sr = librosa.load("./media/" + inp_wav, sr=22050)
        audio_length = len(myrecording)

        myrecording = np.expand_dims(myrecording, 0)

        # wav, _ = librosa.load("./media/" + inp_wav, sr=fs)
        # duration = len(wav) // fs
        # wav = wav[np.newaxis, :]
        ...
        Target_list = open("./Aniaml_demo/Target.txt", 'r', encoding='UTF8')
        Target = []
        for i6 in range(9):
            Targets = Target_list.readline()
            Target.append(Targets)
        Target = np.block(Target)
            ...
            if reg[0]<thread_unknown:
                ...
                if pre<thread_class:
                    ...
                    if not(state in Target):
                        ...
                        animal_totalcount.append(class_convert[Sc])
    ...
    for len1 in length.split(","):
        ...
        if int(len1.split(":")[0]) == 99:
            animal_data.append('')
            animal_color.append('')
        else:
            # animal_data.append('note:"' + labels[int(len1.split(":")[0])] + '"')
            animal_data.append('note:"' + labels[inv_class_convert[int(len1.split(":")[0])]] + '"')
            animal_color.append(int(len1.split(":")[0]))
            animal_count_real.append(labels[inv_class_convert[int(len1.split(":")[0])]])
    ...
    del animal_class[animal_class.index(99)]
    for key in result:
...
#             animal_data.append('note:"' + labels[int(len1.split(":")[0])] + '"')
#             animal_color.append(int(len1.split(":")[0]))
#             animal_count_real.append(labels[int(len1.split(":")[0])])
#         animal_start.append(sumcount)
#         animal_end.append(sumcount)
#
#     animal_class = []
